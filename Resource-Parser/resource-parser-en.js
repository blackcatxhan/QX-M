/** 
☑️ Resource Parser ©𝐒𝐡𝐚𝐰𝐧 ⟦2023-05-02 13:40⟧
----------------------------------------------------------
🛠 Found 𝐁𝐔𝐆 Please feedback: https://t.me/Shawn_Parser_Bot
⛳️ Follow 🆃🅶 related channels: https://t.me/QuanX_API
📖 Tutorial: https://tinyurl.com/2jyygfom
🗣 🆃🄷🄰🄽🄺🅂 🆃🄾  @Jamie CHIEN, @M**F**, @c0lada, @Peng-YM, @vinewx, @love4taylor, @shadowdogy 

🤖 Main functions:
❶ Parse ⟦Server Subscription⟧ in other formats into 𝐐𝐮𝐚𝐧𝐭𝐮𝐦𝐮𝐥𝐭 𝐗 format
☑︎ Support 𝐕2𝐫𝐚𝐲𝐍/𝗦𝗦(𝗥/𝗗)/𝗛𝗧𝗧𝗣(𝗦)/𝗧𝗿𝗼𝗷𝗮𝗻/𝗤𝘂𝗮𝗻𝘁𝘂 𝗺𝘂𝗹𝘁(𝗫)/𝗦𝘂𝗿𝗴𝗲/𝐂𝐥𝐚𝐬𝐡/𝐒𝐡𝐚𝐝𝐨𝐰𝐫𝐨𝐜𝐤𝐞𝐭/𝐋𝐨 𝐨𝐧 format
☑︎ Provides optional personalization parameters (filtering, renaming, etc.) from instruction 1⃣️
❷ Transformation & screening of 𝗿𝗲𝘄𝗿𝗶𝘁𝗲(rewrite) & 𝗳𝗶𝗹𝘁𝗲𝗿(shunt)
☑︎ Used to disable/modify a certain (several) items in the remote reference
☑︎ Parsing usage of 𝐒𝐮𝐫𝐠𝐞/𝐂𝐥𝐚𝐬𝐡 type rules 𝗹𝗶𝘀𝘁 and modules 𝐦𝐨𝐝𝐮𝐥𝐞
----------------------------------------------------------
0️⃣  Add "#" after ⟦Subscription Link⟧ to use, use "&" to connect different parameters
⚠️ ☞ "Your subscription link #emoji=1&tfo=1&in=Hong Kong+Taiwan"
❖ Local resource fragment reference, please fill in parameters such as "#in=xxx&out=yyy" in line ① of the resource fragment
❖ 🚦 Support Chinese, please replace (URL-Encode) when "operating" the following special characters 🚦
   ∎ "+"⇒"%2B", space⇒"%20", "@"⇒"%40", "&"⇒"%26", "."⇒"\.", ","⇒"% 2C"

1️⃣ ⟦𝐬𝐞𝐫𝐯𝐞𝐫 node ⟧ ➠ parameter description:
⦿ emoji=1 (2 for Bank of China equipment)/-1, add/delete region flag in node name;
⦿ udp=1/-1, tfo=1/-1, force open (close) respectively 𝐮𝐝𝐩-𝐫𝐞𝐥𝐚𝐲/𝐟𝐚𝐬𝐭-𝐨𝐩𝐞𝐧;
⦿ uot=1, enable udp-over-tcp=true option (SS(R) only)
⦿ cert=1/-1, respectively enable/disable 𝐭𝐥𝐬 certificate verification (default disabled);
   ❖ csha/psha, tls-cert-sha256 and tls-pubkey-sha256 parameters
   ❖ alpn, specify the alpn parameter of the over-tls type node
⦿ in, out, regex, and regout are respectively the nodes of retention, deletion, regular retention, and regular deletion;
   ❖ in/out is only valid for node name matching, multi-parameter (logical "or") is represented by "+", logical "and" is represented by ".";
   ❖ regex/regout to match the complete information of the node (type, port, encryption, etc.);
   ❖ Demonstration: "in=Hong Kong.0\.2 ratio+Taiwan&out=BGP&regex=iplc"
⦿ rename rename, "old name@new name", "prefix@", "@suffix", use "+" to connect multiple parameters;
   ❖ Delete field: "Field1.Field2☠️", if you want to delete ".", replace it with "\."
   ❖ Demonstration: "rename=Hong Kong@𝐇𝐊+[𝐒𝐒]@+@[1𝐗]+traffic.0\.2☠️"
   ❖ The default emoji takes effect first, if you want to change the order, please use the rrname parameter
⦿ replace Regularly replace fields in nodes, which can be used to rename/change encryption methods, etc.
   ❖ replace=regex1@𝘀𝘁𝗿1+regex2@𝘀𝘁𝗿2
⦿ ptn/npt=1-8, replace node names with English/digital styles ⇒ 🅰/🄰/𝐀/𝗮/𝔸/𝕒/ᵃ/ᴬ, ①\❶\⓵\𝟙\¹\₁\𝟏\𝟷
⦿ delreg, use regular expressions to delete fields in "node name" (⚠️ use with caution)
⦿ aead=-1, turn off the AEAD parameter of Vmess
⦿ host=xxx, modify the existing host, if you want to add a host, please end with ☠️
⦿ obfs=vhttp/shttp, special request to specify obfs=shadowsocks-http or obfs=vmess-http
⦿ tsession=0/1/2, 0/1 means close tls-session-ticket/reuse, 2 means close all
⦿ checkurl=xxx , specify server_check_url parameter
⦿ sort=1/-1/x/parameter rule, sort by node name forward/reverse/random/parameter rule
   ❖ Parameter rules are regular expressions or simple keywords, connected with "<" or ">"
   ❖ sort=🇭🇰>🇸🇬>🇯🇵>🇺🇸 , sort first
   ❖ sort=IEPL<IPLC<BGP , sort later
⦿ info=1, turn on the notification to remind the airport ✈️ flow information (if provided);
⦿ flow=2022-06-02:1000:54, subscription expiration time: total flow: used flow
⦿ placeholder, can be used for operations such as rename/replace
   ❖ $type0/1/2/3/4/5/6/7 is a placeholder, which takes the node type (ss/ssr/vmess, etc.) as an operable parameter, such as
     ∎ rename=@|$type2
     ∎ The styles are "𝐬𝐬", "𝐒𝐒", "🅢🅢", "🆂🆂", "ⓢⓢ", "🅂🅂", "𝕊𝕊", "ˢˢ"
   ❖ $index0/1/2/3/4/5/6/7/8 is a placeholder, which takes the serial number of the node as an operable parameter, such as
     ∎ rename=@「$index1」
     ∎ The styles are 1\①\❶\⓵\𝟙\¹\₁\𝟏\𝟷
   ❖ $emoji1/2 placeholders, take emoji (🇭🇰, etc.) as operable parameters
     ∎ rename=@「$emoji1」
   ❖ $tag placeholder, use the subscribed tag as an operable parameter, such as
     ∎ Can be followed by numbers to add alpha/numeric styles to tags individually
     ∎ rename=@「$tag34」, same style as ptn/npt below
⦿ ⟦Advanced parameters⟧: 𝘀𝗳𝗶𝗹𝘁𝗲𝗿/𝘀𝗿𝗲𝗻𝗮𝗺𝗲, pass in a base64 encoded script, which can be used for more complex [filtering/renaming] requirements
   ❖ Description: https://github.com/KOP-XIAO/QuantumultX/pull/9

2⃣️ ⟦𝐫𝐞𝐰𝐫𝐢𝐭𝐞 Rewrite ⟧/⟦𝐟𝐢𝐥𝐭𝐞𝐫 Split ⟧ ➠ Parameter description:
⦿ in, out, retain/disable related shunt and rewrite rules according to keywords;
⦿ inhn, outhn, "keep/remove" hostnames (𝒉𝒐𝒔𝒕𝒏𝒂𝒎𝒆);
...
⦿ regex/regout, regular retention/deletion, please toss the regular expression by yourself;
   ❖ Can be used together with in(hn)/out(hn), in(hn)/out(hn) will be executed first;
   ❖ Valid for both 𝒉𝒐𝒔𝒕𝒏𝒂𝒎𝒆 & 𝐫𝐞𝐰𝐫𝐢𝐭𝐞/𝐟𝐢𝐥𝐭𝐞𝐫 (use with caution)
⦿ policy parameter, used to specify policy group directly, or generate policy group for 𝐒𝐮𝐫𝐠𝐞 type 𝗿𝘂𝗹𝗲-𝘀𝗲𝘁 (default "𝐒𝐡𝐚𝐰𝐧" policy group);
⦿ pset=regex1@policy1+regex2@policy2, specify different policy groups for different keywords (regular expressions allowed) in the same diversion rule;
⦿ replace parameter, regular replacement 𝐟𝐢𝐥𝐭𝐞𝐫/𝐫𝐞𝐰𝐫𝐢𝐭𝐞 content, regex@newregex;
   ❖ Replace the script in Taobao Price Comparison with the lite version (if there is a script of this version)
     ∎ replace=(price)(.*)@$1_lite$2
⦿ dst=rewrite/filter, respectively convert 𝐦𝐨𝐝𝐮𝐥𝐞&𝗿𝘂𝗹𝗲-𝘀𝗲𝘁 into rewrite/shunt;
   ❖ ⚠️ Convert 𝐦𝐨𝐝𝐮𝐥𝐞 to rewrite and 𝗿𝘂𝗹𝗲-𝘀𝗲𝘁 to shunt by default
   ❖ ⚠️ When converting url-regex in 𝗿𝘂𝗹𝗲-𝘀𝗲𝘁 to rewrite, you must add dst=rewrite;
   ❖ ⚠️ When converting the shunt rules in 𝐦𝐨𝐝𝐮𝐥𝐞, you must add dst=filter
⦿ cdn=1, convert the address of the github script to fastly.jsdelivr.net/gh
⦿ fcr=1/2/3, add force-cellular/multi-interface/multi-interface-balance parameters to split rules, force mobile data/mixed data/load balancing
⦿ via=interface, add via-interface parameter for splitting rules, 0 means via-interface=%TUN%
⦿ relay=target policy name, convert node subscriptions into ip/host rules in batches, used to implement proxy chain

3⃣️ Other parameters
⦿ Notification parameter ntf=0/1, used to close/open resource resolver notification
   ❖ 𝗿𝗲𝘄𝗿𝗶𝘁𝗲/𝗳𝗶𝗹𝘁𝗲𝗿 The notification prompt is "on" by default to prevent accidental deletion of rules
   ❖ 𝘀𝗲𝗿𝘃𝗲𝗿 resource resolution is "closed" by default
⦿ type parameter type=domain-set/rule/module/list/nodes
   ❖ When the parser fails to recognize the type correctly, you can try to use this parameter to force
⦿ Hidden parameter hide=0, disable splitting/rewriting of filtering, default way is to delete
⦿ profile=111 , URL-Scheme add remote resource in QuanX type configuration
----------------------------------------------------------
*/
...
⚠️⚠️If it prompts "No custom parser", please long press the icon in the lower right corner and then click the refresh button on the left to update resources, and exit the app in the background until the parser description appears

------------------------------
*/

//beginning The parser is in normal use, debug comment this part

let [link0, content0, subinfo] = [$resource.link, $resource.content, $resource.info]
let version = typeof $environment != "undefined" ? Number($environment.version.split("build")[1]): 0 // version number
let Perror = 0 //error type

const subtag = typeof $resource.tag != "undefined" ? $resource.tag : "";
////// Sand sculpture situation of non-raw link
content0 = content0.indexOf("DOCTYPE html") != -1 && link0.indexOf("github.com") != -1 ? ToRaw(content0) : content0 ;
//ends normal use part, debug comment this part


var para = /^(http|https)\:\/\//.test(link0) ? link0 : content0.split("\n")[0];
var para1 = para.slice(para.indexOf("#") + 1).replace(/\$type/g,"node_type_para_prefix").replace(/\$emoji/g,"node_emoji_flag_prefix").replace(/\$tag/g,"node_tag_prefix").replace(/\$index/g,"node_index_prefix") //Prevent "#" from appearing in other positions in the parameter
var mark0 = para.indexOf("#") != -1 ? true : false; //Whether there are parameters to parse
var Pinfo = mark0 && para1.indexOf("info=") != -1 ? para1.split("info=")[1].split("&")[0] : 0;
var ntf_flow = 0;
//Commonly used amount
const Base64 = new Base64Code();
const escapeRegExp = str => str.replace(/[-[\]{}()*+?.,\\^$|#\s]/g, '\\$&'); //Handle special symbols for regular matching
...
if(version == 0) { $notify("⚠️ Please update Quantumult X to the latest store version\n","🚦The current version may not be able to use some functions normally","\n👉Click the jump store link to update",update_link) }
...
var Pregout = mark0 && para1.indexOf("regout=") != -1 ? decodeURIComponent(para1.split("regout=")[1].split("&")[0]).replace(/\，/g,",") : null; //server regular deletion parameter
...
var Preplace = mark0 && para1.indexOf("replace=") != -1 ? para1.split("replace=")[1].split("&")[0] : null; //filter/rewrite regular replacement
...
var Pptn = mark0 &&para1.indexOf("ptn=") != -1 ? para1.split("ptn=")[1].split("&")[0] : ""; //fancy english characters
var Pnptn = mark0 &&para1.indexOf("npt=") != -1 ? para1.split("npt=")[1].split("&")[0] : ""; //fancy numbers
var Pcdn = mark0 &&para1.indexOf("cdn=") != -1 ? para1.split("cdn=")[1].split("&")[0] : "";
let [flow, exptime, errornode, total] = "";
var Pdel = mark0 && para1.indexOf("del=") != -1 ? para1.split("del=")[1].split("&")[0] : 0; //remove duplicate nodes
var typeU = mark0 && para1.indexOf("type=") != -1 ? para1.split("type=")[1].split("&")[0] : "";
var Pfcr = mark0 && para1.indexOf("fcr=") != -1 ? para1.split("fcr=")[1].split("&")[0] : ""; // force-cellular and other parameters
var Pvia = mark0 && para1.indexOf("via=") != -1 ? para1.split("via=")[1].split("&")[0] : ""; // via-interface parameter
var Paead = mark0 && para1.indexOf("aead=") != -1 ? para1.split("aead=")[1].split("&")[0] : ""; // vmess aead parameter
var Phost = mark0 && ( para.indexOf("#host=") != -1 || para.indexOf("&host=") != -1) ? (para.indexOf("#host=")!=-1? para.split("#host="): para.split("&host="))[1].split("&")[0] : ""; // host obfuscation parameters
var Pcsha256 = mark0 && para1.indexOf("csha=") != -1 && version >= 646? para1.split("csha=")[1].split("&")[0] : ""; // cert-sha256 obfuscation parameters
var Ppsha256 = mark0 && para1.indexOf("psha=") != -1 && version >= 646? para1.split("psha=")[1].split("&")[0] : ""; // pubkey-sha256 obfuscation parameters
var typeQ = $resource.type? $resource.type:"unsupported"   //return field type parameter
var PRelay =mark0 && para1.indexOf("relay=") != -1 ? decodeURIComponent(para1.split("relay=")[1].split("&")[0]) : ""; // node relay parameter, Used to implement the proxy chain function
var PUOT = mark0 && para1.indexOf("uot=") != -1 && version >= 665? para1.split("uot=")[1].split("&")[0] : ""; // node udp-over-tcp turn on
var PcheckU = mark0 && para1.indexOf("checkurl=") != -1 ? decodeURIComponent(para1.split("checkurl=")[1].split("&")[0]) : ""; // node server_check_url parameter
typeQ = PRelay!=""? "server":typeQ
var typec="" //check result type
var Pflow=mark0 && para1.indexOf("flow=") != -1 ? para1.split("flow=")[1].split("&")[0] : 0; // traffic time and other parameters
var PProfile = mark0 && para1.indexOf("profile=") != -1 ? para1.split("profile=")[1].split("&")[0] : 0; // Import complete configuration parameters via URL-Scheme
var Palpn = mark0 && para1.indexOf("alpn=") != -1 && version >= 712? para1.split("alpn=")[1].split("&")[0] : ""; // over-tls type, alpnparameter
var Pobfs = mark0 && para1.indexOf("obfs=") != -1 && version >= 770? para1.split("obfs=")[1].split("&")[0] : ""; // Specify obfs=xx-http type for special cases
...
// 0/1 means close session-ticket/reuse, 2 means close all.

var RegoutList= [] ;//Used for regoutparameter deletion reminder
...
//Function to avoid json undefined errors
...
function Parser() {
  ...
  if (type0 != "web" && type0 != "wrong-field" && type0 != "JS-0"){
    try {
      ...
      if(Perror == 0) {
      $notify("❌ There was an error in parsing", "⚠️ Please click on the notification to send a subscription link for feedback", err, bug_link);
    }
    }
  } else if (type0 == "wrong-field"){
    if (version >= 670 && typec!="") { //try to jump to the correct type
      RLink0[Field[typec]].push($resource.link+", opt-parser=true, tag=Add resources next time 🉑️Longer ❤️8⃣️") //  Jump URI-Scheme
    ...
    $notify( "⚠️ Please click the notification to jump and try to add it to the correct type", "❌ The detection type ["+typec+"]"+" conflicts with the filling type "+"["+typeQ+"]", "If the jump is added Failed, please send link feedback parser bot\n"+$resource.link, bug_linkx)
    } else {//old version
    $notify("❌ Detection type 「"+typec+"」"+" conflicts with target type "+" 「"+typeQ+"」", "⚠️ Please check the link content by yourself, or click the notification to send the link for feedback", $resource.link, bug_link)
    }
    total=""
  } else {
    total=""
...
if (typeof($resource)!=="undefined" && PProfile == 0) {
  ...
  $notify("⚠️Please ignore the error message, click this notification to jump", "Add valid remote resources in the configuration👇 ["+ PProfile+"]", ADDres,openlink)
...
function ParseUnknown(cnt){
  try {
    cnt = JSON.parse(cnt)
    if(cnt) {
      $notify("⚠️ The content returned by the link is not a valid subscription "+ "⟦" + subtag + "⟧","⁉️ Please check the original link yourself and return the content 👇️👇️",JSON.stringify(cnt), bug_link)
    }
    
  } catch(err) {
    $notify("😭 Unable to recognize the subscription format: " + "⟦" + subtag + "⟧", "⚠️ will be directly imported into Quantumult X \n If you think it is a BUG, please click the notification to jump and send a link feedback", "The link returns content: \n"+cnt, bug_link);
  }
}
...
function ResourceParse() {
  //Preprocessing, shunting/rewriting and other processing are completed
  if (type0 == "Subs-B64Encode") { // subs2QX is responsible for all node conversions
    ...
    //2023-03-06 Consider a mashup of module overrides and quanx type overrides
    flag = 2 
    //total = SGMD2QX(content0) // convert
    total = Rewrite_Filter(isQuanXRewrite(content0.split("\n")), Pin0, Pout0,Preg,Pregout);//Rewrite_Filter(total, Pin0, Pout0,Preg,Pregout); // filter filter
    if (Preplace) { total = ReplaceReg(total, Preplace) }
    total = total.filter( (ele,pos)=>total.indexOf(ele) == pos); //rewrite duplicate check
    ...
    if (Preg && total.length!=0) { // Regular filter rule filter
    total = total.map(Regex).filter(Boolean) 
    RegCheck(total, "分流引用", "regex", Preg)
  } 
    if (Pregout && total.length!=0) { // Regex delete rule filter
    ...
    $notify("‼️ Quote" + "⟦" + subtag + "⟧" + "The returned content is empty", "⁉️ Click the notification to confirm whether the link is invalid", para.split("#")[0], nan_link);
    flag = 0;
    ...
    $notify("⚠️ This link is for node subscription, please click this notification to jump to add", url, ADDres0,openlink)
    flag = -1
    total = ""
    ...
    PProfile = "111" //All sections are added by default
    Profile_Handle()
    openlink = {"open-url": ADDres}
    $notify("⚠️ This link is a complete configuration file, please click this notification to jump", "Add valid remote resources in the configuration 👇 ["+ PProfile+"]", ADDres, openlink)
    flag = -1;
    total = ""
  } else if (type0 == "JS-0") {
    total = ""
  ...
  if (flag == 1) { //server Unified processing of types
    ...
    //script rename Placed before other parameters
    if (Psrename) { total = RenameScript(total, Psrename) }
    if (Preplace) { // The server type can also be renamed with replace parameter
    ...
    if (total.length > 0){
      ...
      total = total.map(type_handle).map(emoji_prefix_handle).map(tag_handle) //Various node name operations
      if (Psort0) { //Sort operation
        total = QXSort(total, Psort0);
      }
      total = para1.indexOf("node_index_prefix")!=-1 ?index_handle(total):total // node serial number operation
      //$notify("before","haha",total)
      total = TagCheck_QX(total).join("\n") //node name check
      if (PUOT==1) { total = total.split("\n").map(UOT).join("\n")}
      if (Pcnt == 1) {$notify("⟦" + subtag + "⟧"+"finally return content after parsing" , "Number of nodes: " +total.split("\n").length, total)}
      total = PRelay==""? Base64.encode(total) : ServerRelay(total.split("\n"),PRelay) //Mandatory node type base64 encryption before importing Quantumult X, if it is a relay, it will be converted into stream type
      if(Pflow==1) {
        //$notify("添加traffic information","xxx","xxxx")
        $done({ content: total, info: {bytes_used: 3073741824, bytes_remaining: 2147483648, expire_date: 1854193966}});
      //$notify("done?","strange")
      } else { $done({ content: total });}
    } else {
      if(Perror == 0) {
      $notify("❓❓ Friendly reminder ➟ "+ "⟦" + subtag + "⟧", "⚠️⚠️ No valid content after parsing", "🚥🚥 Please check the relevant parameters yourself, or click the notification to jump and send link feedback", bug_link)
    }
      total = errornode
      $done({ content: errornode })
    }
  } else if (flag == 0){ //empty/wrong type
    total = errornode
    $done({ content: errornode })
  ...
  if (Pcnt == 1 && flag !=1) {$notify("After parsing, the final return content" , "total quantity： " +total.split("\n").length, total)}
  return total
...
function SubFlow() {
  if (Pinfo == 1 && subinfo) {
    ...
    var left = "residual flow: " + ((parseFloat(sinfo.split("total=")[1].split(",")[0]) / (1024 ** 3)) - ((parseFloat(sinfo.indexOf("upload")!=-1?sinfo.split("upload=")[1].split(",")[0]:"0") + parseFloat(sinfo.split("download=")[1].split(",")[0])) / (1024 ** 3))).toFixed(2) + "GB"
    if (sinfo.indexOf("expire=") != -1) {
      ...
      var mth = epr.getMonth() + 1 < 10 ? '0' + (epr.getMonth() + 1) : (epr.getMonth() + 1);  // Get the month (0-11, 0 represents January, remember to add 1 when using it)
      var day = epr.getDate() < 10 ? "0" + (epr.getDate()) : epr.getDate();
      epr = "Expiration: " + year + "-" + mth + "-" + day
    } else {
      epr = ""; //"Expiration: ✈️ The information is not provided" //Show subscription link without Expiration
    }
    var message = total + "\n" + usd + ", " + left;
    ntf_flow = 1;
...
//  } else if (Pinfo ==1){
//    $notify("traffic information: ⟦" + subtag + "⟧", "", "⚠️ 该订阅链接未返回任何traffic information", subinfo_link)
//  }
}
...
function flowcheck(cnt) {
  ...
  flow = flow? flow:"⚠️ The subscription did not return any traffic information"
  exptime = exptime? exptime:"⚠️ The subscription did not return package time information"
    if (flow != "") { $notify("traffic information: ⟦" + subtag + "⟧", flow, exptime, subinfo_link1) }
}

// regex 后的检查
function RegCheck(total, typen, paraname,regpara) {
  if(total.length == 0){ 
    $notify("‼️ " + typen + " ➟ " + "⟦" + subtag + "⟧", "⛔️ Filter regularity: " + paraname + "=" + regpara, "⚠️ The remaining items after filtering are 0️⃣, please check the regular parameter and the original Link", nan_link)
  }else if((typen != "node订阅" && Pntf0 !=0) || (typen == "node订阅" && Pntf0 ==1)){
    var nolist = total.length <= 10 ? emojino[total.length] : total.length
    $notify("🤖 " + typen + " ➟ " + "⟦" + subtag + "⟧", "⛔️ filter regex: " + paraname + "=" + regpara, "⚠️ the following matches are left after filtering" + nolist + "\ n ⨷ " + total.join("\n ⨷ "), sub_link)
  }
}
//判断订阅类型
function Type_Check(subs) {
    ...
    if ( (subs.indexOf(html) != -1 || subs.indexOf("doctype html") != -1) && link0.indexOf("github.com" == -1)) {
      $notify("‼ ️ The link returns invalid content"+ " ➟ " + "⟦" + subtag + "⟧", "⁉️ Click the notification to confirm whether the link is invalid\n"+link0, "The returned content is as follows ⬇️:\n"+subs, nan_link);
      type = "web";
    } else if (typeU == "nodes" && typeQ=="server") { //指定为node类型
      type = (typeQ == "unsupported" || typeQ =="server")? "Subs":"wrong-field"
    } else if (ClashK.some(NodeCheck) || typeU == "clash"){ // Clash 类型node转换
      ...
      Perror = 1 ; // 无需反馈
      $notify("⚠️ The content of the link you imported is a JS script", "There is no rewrite rule in the 🚥 script, so it cannot be parsed and used", " Please ⚠️Don't ⚠️ come to the parser 🤖️ for feedback \n"+link0)
      type = "JS-0"
  ...
  // 用于通知判断类型，debug
  if(typeU == "X"){
    $notify("The link decision type",type+" : " +typec, subs)
  }
  //$notify(type)
    return type
}

// 检查node名字(重复以及空名)等QuanX 不允许的情形，以及多个空格等“不规范”方式
function TagCheck_QX(content) {
  ...
  console.log(content)
    ...
    //$notify("","",Olist)
    var [Nlist, nmlist] = [ [], [] ]
    var [nulllist,duplist] = [ [], [] ]; //记录空名字node&重名node
    var no=0 ;
    ...
    // 增加 server_check_url parameter
    if (PcheckU != "") {
      Nlist = Nlist.map(Add_URL)
    }
    if (nulllist.length >= 1) {
        no = nulllist.length <= 10 ? emojino[nulllist.length] : nulllist.length;
        $notify("⚠️ Quoting " + "⟦" + subtag + "⟧" + " contains " + no + "empty node names", "✅ node "type+IP" has been set to node name", " ⨁ " + nulllist.join("\n ⨁ "), nan_link)
    }
    if (duplist.length >= 1) {
        no = duplist.length <= 10 ? emojino[duplist.length] : duplist.length;
      if (Pdel!=1 && Pntf0 != 0){
        $notify("⚠️ Quoting " + "⟦" + subtag + "⟧" + " contains " + no + "nodes with duplicate names", "✅ Added number distinction, please add parameter del=1 to delete:", " ⨁ " + duplist.join("\n ⨁ "), nan_link)
      } else if (Pdel ==1 && Pntf0 != 0) {
        $notify("⚠️ Quoting " + "⟦" + subtag + "⟧" + " contains " + no + " nodes with duplicate names ", "❌️ All have been deleted, if you want to keep it, please remove parameter del=1:", " ⨁ " + duplist.join("\n ⨁ "), nan_link)
      }
    }
    return Nlist
}

// 为node添加 server-check-urlparameter
...
// 用于单条 URI 的 tag parameter, 直接指定node名
...
function ToRaw(cnt) {
  ...
  if (cnt) {
    $notify( "⚠️⚠️ Will try to parse this resource" + "⟦" + subtag + "⟧" , "🚥 Please use GitHub's raw link correctly" , "❌ Your link: "+link0+"\n✅ correct link："+rawlink, {"open-url":rawlink})
  } else if(content0.indexOf("gridcell")!=-1) {
    $notify( "⚠️⚠️ Parsing this resource" + " ⟦" + subtag + "⟧ failed" , "🚥 your link appears to be a directory, not a file" , "❌ your link："+link0, {"open-url":link0})
  }
  return cnt
}
...
function HOST_Handle(cnt,phost) {
  ...
  if (phost.indexOf("☠️") == -1) { //只替换已有host类型
    ...
    if (/-host\s*\=/.test(cnt)) {// 如已有 host parameter
      cnt = cnt.replace(/host\s*\=(.*?)\,/,phost+", ")
...
function OBFS_Handle(cnt,pobfs) {
  ...
  if (/obfs\s*\=\s*http/.test(cnt) && pobfs!="invalid") { //只替换有 obfs=http parameter
    cnt = cnt.replace(/obfs\s*\=\s*http/,pobfs)
  }
  return cnt
}

// 指定node tls-no-session-ticket/reuse parameter
...
// 指定 cert-sha256 pubkey-sha256 parameter
...
function SCP2QX(subs) {
  ...
  for (var i = 0; i < subs.length; i++) {
    try {
      ...
      $notify("❌️An error occurred while parsing this article, it has been ignored",subs[i],err)
    }
  }
  return nrw
}
...
function Rewrite_Filter(subs, Pin, Pout,Preg,Pregout) {
        ...
        if (subi != "" && (subi.indexOf(" url ")!=-1 || subi.indexOf(" url-and-header ")!=-1 || /^hostname\=/.test(subii))) {
            ...
            if (noteK.some(notecheck)) { // 注释项跳过 
                ...
                if (outflag == 1 || inflag == 0) {
                    ...
                    Nlist.push(subi);
                } else if (outflag == 2 && inflag != 0) { //无 out parameter && in 未排除
                    Nlist.push(subi);
                }
            }
        }
    ...
    if (Pntf0 != 0) {
        ...
        if (Pin0 && no1write != " 0️⃣ ") { //有 in parameter就通知保留项目
          if (Pout!=0) {
            $notify("🤖 " + "rewrite reference➟ " + "⟦" + subtag + "⟧", "⛔️ filter parameter: " + pfi + pfo, "☠️ keep the following matches in rewrite" + no1write + ":" + "\n ⨷ " + Nlist.join("\n ⨷ "), rwrite_link)
          }
        } else if (dwrite.length > 0) {
          if (Pout0!=0) {
            $notify("🤖 " + " rewrite reference ➟ " + "⟦" + subtag + "⟧", "⛔️ filter parameter: " + pfi + pfo, "☠️ The following " + nowrite + " matches are disabled in rewrite rewrite:" + "\n ⨷ " + dwrite.join("\n ⨷ "), rwrite_link)
          }
        }
    }
    if (Nlist.length == 0 ) { 
      if ((Pin0 || Pout0 || Phin0 || Phout0 || Pregout || Preg)) {
        $notify("🤖 " + "Rewrite references ➟ " + "⟦" + subtag + "⟧", "⛔️ filter parameter: " + pfi + pfo, "⚠️ The number of remaining rewrite rules after filtering is 0️⃣, please check the parameter and the original link", nan_link) 
      } else {
        $notify("🤖 " + "Rewrite reference ➟ " + "⟦" + subtag + "⟧", "⛔️ After parsing, the number of rewrite rules is 0️⃣ " , "⚠️ Please check the parameter and the original link content", nan_link) 
...
function HostNamecheck(content, parain, paraout) {
    ...
    for (var i = 0; i < hname.length; i++) {
        ...
        if (paraout && paraout != "") { //存在 out parameter时
            if (!paraout.some(excludehn)) { //out 未命中🎯️
                ...
                } else { nname.push(hname[i]) } //无in parameter    
            } else { dname.push(hname[i]) } //out parameter命中
        } else if (parain && parain != "") { //不存在 out，但有 in parameter时
            if (parain.some(excludehn)) { //Pin 命中🎯️
                nname.push(hname[i])
    ...
    if (Pntf0 != 0) {
        if (paraout || parain) {
            ...
            if (parain && no1name != " 0️⃣ ") {
                $notify("🤖 " + "Rewrite reference ➟ " + "⟦" + subtag + "⟧", "⛔️ filter parameter: " + pfihn + pfohn, "☠️ The following " + no1name + " matches are reserved in hostname hostname: " + "\n ⨷ " + nname.join(","), rwhost_link)
            } else if (dname.length > 0) {
                $notify("🤖 " + "Rewrite references ➟ " + "⟦" + subtag + "⟧", "⛔️ filter parameter: " + pfihn + pfohn, "☠️ The following occurrences of " + noname + " were removed from hostname hostname: " + "\n ⨷ " + dname.join(","), rwhost_link)
            }
        }
    }
    if (nname.length == 0) {
        $notify("🤖 " + "rewrite reference ➟ " + "⟦" + subtag + "⟧", "⛔️ filter parameter: " + pfihn + pfohn, "⚠️ there are 0️⃣ items left in hostname, please check the parameter and the original link", nan_link)
    }
    ...
    if (Tout != "" && Tout != null) { // 有 out parameter时
        ...
        for (var i = 0; i < cnt.length; i++) {
            ...
            if (Tout.some(exclude) && !RuleK.some(CommentCheck) ) {
              // 2022-12-15 删除 && RuleK2.some(RuleCheck) 判断条件，以免 list/provider 中parameter上生效
        ...
        if (dlist.length > 0) {
            if (Pntf0 != 0) { $notify("🤖 " + "Split references ➟ " + "⟦" + subtag + "⟧", "⛔️ Disabled: " + Tout, "☠️ The following matching rules are disabled:" + no + " + "\n ⨷ " + dlist.join("\n ⨷ "), rule_link) }
        } else { $notify("🤖 " + "Split reference ➟ " + "⟦" + subtag + "⟧", "⛔️ Disabled: " + Tout, "⚠️ No match found, please check parameter or original link", nan_link) }
        if (Tin != "" && Tin != null) {  //有 in 跟 out parameter时
            if (nlist.length > 0) {
                var noin0 = nlist.length <= 10 ? emojino[nlist.length] : nlist.length
                if (Pntf0 != 0) {
                    $notify("🤖 " + "Split reference ➟ " + "⟦" + subtag + "⟧", "✅ Reserved:" + Tin, "🎯 The following matching rules have been reserved: " + noin0 + ":" + "\n ⨁ " + nlist.join("\n ⨁ "), rule_link)
                }
            } else {
                $notify("🤖 " + "Split Quote➟ " + "⟦" + subtag + "⟧", "✅ Reserved:" + Tin + ",⛔️ Disabled: " + Tout, "⚠️ The number of remaining rules after filtering is 0️⃣, please check parameter and original link", nan_link)
            }
        } else {// if Tin (No Tin)
            if (nlist.length == 0) {
                $notify("🤖 " + "Split reference ➟ " + "⟦" + subtag + "⟧", "⛔️ Disabled: " + Tout, "⚠️ The remaining rules after filtering are 0️⃣, please check the parameter and the original link", nan_link)
            }
        }
      nlist =Phide ==1? nlist : [...dlist,...nlist]
        ...
        if (nlist.length > 0) {
            ...
            if (Pntf0 != 0) {
                $notify("🤖 " + "Split Quote ➟ " + "⟦" + subtag + "⟧", "✅ Reserved:" + Tin, "🎯 The following " + noin + " matching rules have been reserved:" + "\n ⨁ " + nlist.join("\n ⨁ "), rule_link)
            }
        } else { $notify("🤖 " + "Split Quote ➟ " + "⟦" + subtag + "⟧", "✅ Reserved:" + Tin, "⚠️ The number of remaining rules after filtering is 0️⃣, please check the parameter and the original link", nan_link) }
      nlist =Phide ==1? nlist : [...dlist,...nlist]
      //return nlist;
    } else {  //if Tin
            ...
            try {
              if (Pdbg) {$notify(i, type, list0[i])}
                if (type == "vmess" && (list0[i].indexOf("remark=") == -1 && list0[i].indexOf("remarks=") == -1) && !/(obfs|alterId|type)\=/.test(list0[i])) {
                  ...
                  Perror = 1 ; // 无需反馈
                  $notify("⚠️ Quantumult X does not support Vless type node yet", "Please ⚠️Don't⚠️ come to the parser🤖️ to give feedback",list0[i])
                }
            } catch (e) {
                failedList.push(`<<<\nContent: ${list0[i]}\nError: ${e}`)
            }
            if (Paead != "") {node = AeadVmess(node,Paead)} // vmess 类型 aead 处理
            if (Phost != "") {node = HOST_Handle(node,Phost)} // host parameter修改
            if (Pobfs != "") {node = OBFS_Handle(node,Pobfs)} // obfs parameter修改
            if (Psession != "") { node = Session_Handle(node,Psession)} // tls-session parameter
            if (Pcsha256 != "" || Ppsha256 != "") {
            node = SHA256_Handle(node,Pcsha256,Ppsha256)} // Sha256 parameter
            if (Palpn !="") { node = ALPN_Handle(node,Palpn)} // alpn parameter
            node = TLS_Check(node)
...
//新版本tls 的检验（存在sha256 parameter时）
...
function QX_TLS(cnt,Pcert0,PTls13) {
  ...
  if(cnt.indexOf("tls-verification") != -1){ // 已有tlsparameter时, 如用户不指定，则不做处理
  ...
  }else if(cnt.indexOf("obfs=over-tls")!=-1 || /over\-tls\s*\=\s*true/.test(cnt) || cnt.indexOf("obfs=wss")!=-1){ //未包含tlsparameter时
...
function HPS2QX(subs, Ptfo, Pcert0, PTls13) {
    ...
    if (server != "") {
      if (server.indexOf("@")!=-1) {
        ...
        var thost = subs.indexOf("peer=")!= -1? "tls-host=" + subs.split("peer=")[1].split("#")[0].split("&")[0] : "" // 存在peersparameter时 https://b64(ipport)?peer=xxx#server-remarks
...
function VR2QX(subs, Pudp, Ptfo, Pcert0, PTls13) {
  ...
  if (subs.indexOf("#")==-1) {
    tag = /remarks*=/.test(subs)? "tag=" + decodeURIComponent(subs.split(/remarks*=/)[1].split("&")[0]) : "tag="+server.split("@")[1] //部分无node名的情况
  } else {
  ...
  if (obfs == "none") { //
    ...
    Perror = 1 // 不需要反馈的类型
    if (Pntf0!=0) {
    $notify( "⚠️ Quantumult X does not currently support this type of node", "The following grpc|h2 vmess node has been ignored",subs)
  }
    pdrop = 1
  }
  if (obfs.indexOf("obfs=over-tls") != -1 || obfs.indexOf("obfs=wss") != -1) {
...
function Fobfs(jsonl, Pcert0, PTls13) {
  ...
  if (jsonl.net == "ws" && jsonl.tls == "tls") {
    ...
    Perror = 1
    $notify("⚠️ Quantumult X does not support this type of node", "vmess + " + jsonl.net, JSON.stringify(jsonl))
...
//node过滤，使用+连接多个关键词(逻辑"或"):in 为保留，out 为排除, "与"逻辑请用符号"."连接
function Filter(servers, Pin, Pout) {
    ...
    for (var i = 0; i < servers.length; i++) {
        ...
        } else { Delist.push(servers[i].replace(/ /g, "").split("tag=")[1]) } //记录未被保留node
    }//for
    ...
    if (Pntf0 == 1 && Delist.length >= 1) {//通知部分
        if (Pin && no1 > 0) { //有 in parameter就通知保留部分
            $notify("👥 Quote" + "⟦" + subtag + "⟧" + "Start node filtering", "🕹 filter keywords: " + pfi + pfo, "☠️ The following are reserved" + no1 + "node\n" + Nname.join(", "), sub_link);
        } else if (Pout && no > 0) {
            $notify("👥 citation" + "⟦" + subtag + "⟧" + " start node filtering", "🕹 filter keywords: " + pfi + pfo, "☠️ deleted the following " + no + "node\n" + Delist.join(", "), sub_link);
        }
    } else if (no1 == 0 || no1 == null) { //无剩余node时强制通知
        $notify("‼️ ⟦" + subtag + "⟧" + "The number of nodes after filtering is 0️⃣", "⚠️ Please check the original link and filter parameter by yourself", link0, sub_link);
    }
    return Nlist
}

function FilterScript(servers, script) {
    $notify("🤖 Enable scripts for filtering", "", script);
    try {
        const $ = Tools();
        eval(script);
        // extract server tags
        ...
        $notify("❌ Error in script filter", "", err);
        return servers;
    }
}
...
function SSR2QX(subs, Pudp, Ptfo) {
    ...
    var obfshost = '';
    var oparam = '';
    if (cnt.split(":").length <= 8) { //排除难搞的 ipv6 node
        type = "shadowsocks=";
...
function SS2QX(subs, Pudp, Ptfo) {
  ...
  if (cnt.split(":").length <= 6) { //排除难搞的 ipv6 node
    type = "shadowsocks=";
...
function SSD2QX(subs, Pudp, Ptfo) {
    ...
    for (var i in cnt.servers) {
        ...
        if (ip.indexOf(".") > 0) { //排除难搞的 ipv6 node
...
try {
  ...
  cntis = cnti.split(",").filter(Boolean).map(item => item.trim()) //防止node名中有,符号而导致的错误情况
  tagfix = ""
  cntii = ""
  for (i in cntis) {
  ...
  if(Perror == 0) {
  $notify("❌ There was an error in parsing, and this entry has been ignored", "⚠️ Please click the notification to send a subscription link for feedback", cntf+"\n"+ err, bug_link);
}
}
  return ""
}

// 用于过滤非node部分（比如整份配置中其它内容）,同时纠正部分不规范的写法(没有把 tag 写在最后)
...
function RenameScript(servers, script) {
    $notify("🤖 Enable scripts for renaming", "", script);
    try {
        const $ = Tools().rename;
        // extract server tags
        const nodes = Tools().getNodeInfo(servers);
        ...
        $notify("❌ Error renaming script", "", err);
        return servers;
    }
...
//Loon 的 ssr 部分
//# SSR 格式：名称=协议类型,地址,端口,加密方式,密码,协议类型,{协议parameter},混淆类型,{obfuscation parameters}
...
function Clash2QX(cnt) {
  ...
  for (i=0; i<nl; i++){
    try{
      ...
      $notify(`⚠️The node is parsed incorrectly and has been ignored for the time being`, `click on the notification and send a link to feedback to the bot`,JSON.stringify(node),bug_link )
      $notify(`⚠️The error content is as follows`, `you can copy the error content to the feedback bot`,JSON.stringify(node)+"\n\n"+e)
    }
  }
...
// UDP/TFO parameter (强制 surge/quanx 类型转换)
