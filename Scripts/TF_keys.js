...
*********************************
QuantumultXviết lại người giới thiệu：
...
if (reg1.test($request.url)) {
  ...
  if ($prefs.valueForKey("request_id") !== null) {
    $notify("TestFlight Auto Join", "Thông tin thu được thành công", "");
  } else {
    $notify("TestFlight Auto Join", "Thu thập thông tin không thành công", "Vui lòng thêm testflight.apple.com");
  }
  $done({});
  ...
  $notify("TestFlight Auto Join", `Đã thêm APP_ID: ${id}`, `ID hiện tại: ${appId}`);
  $done({});
}
